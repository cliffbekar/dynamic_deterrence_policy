%EPDDHetq.m
%Person specific reference groups for q for Section 3.1.2 of Online Appendix  
...
for r=1:NN
    ...
    while ((cc<1) && (b<MM))
        ...
        swi=zeros(Block,1);
        swic=zeros(Block,1);
        vz=zeros(Block,N,1);vn=zeros(Block,N,1);vnn=zeros(Block,N,1);svn=zeros(Block,1);
        az=zeros(Block,N,1);an=zeros(Block,N,1);ann=zeros(Block,N,1);san=zeros(Block,1);
        eta=zeros(Block,N,1);etam=zeros(Block,1);
        R=zeros(Block,1);
        gbar=zeros(Block,N);
        gbar3=zeros(Block,1);
        A=zeros(Block,1);
        ...
        for t=1:Block            
            ...
            if t>1
                for n=1:N
                    for i=1:refsize+1
                        vnn(t,n)=vnn(t,n)+vn(t-1,refg(n,i));
                        ann(t,n)=ann(t,n)+an(t-1,refg(n,i));
                    end
                end
            end
            if (b<2)
                if (t<Z+1)
                    for z=1:Z
                        for n=1:N
                            vz(z,n)=Z*unifrnd(0,N);
                            az(z,n)=Z*unifrnd(0,vz(z));
                        end
                    end
                else
                    for n=1:N
                        %for i=1:refsize
                            vz(t,n)=sum(vnn(t-Z:t-1,n));
                            az(t,n)=sum(ann(t-Z:t-1,n));
                        %end
                    end
                end
            else
                if t<1+Z
                    if t<2
                        for n=1:N
                            vz(t,n)=sum(v(t+(b-1)*Block-Z:t+(b-1)*Block-1));
                            az(t,n)=sum(a(t+(b-1)*Block-Z:t+(b-1)*Block-1));
                        end
                    else
                        for n=1:N
                            vz(t,n)=sum(v(t+(b-1)*Block-Z-1:(b-1)*Block))+sum(vw(t-(Z-(Z-(t-1))):t-1));
                            az(t,n)=sum(a(t+(b-1)*Block-Z-1:(b-1)*Block))+sum(aw(t-(Z-(Z-(t-1))):t-1));  
                        end
                    end
                else
                    for n=1:N
            ...
            for n=1:N
                eta(t,n)=(aa+az(t,n))/(aa+bb+vz(t,n));
                if eta(t,n)*F<=g(t,n)
                    vn(t,n)=1;
                    vw(t)=vw(t)+1;
                    gbar(t,n)=g(t,n);
...
[acf38,lag38]=autocorr(vcon(38,Block-10000:Block-5000));
[acf39,lag39]=autocorr(vcon(39,Block-10000:Block-5000));
[acf40,lag40]=autocorr(vcon(40,Block-10000:Block-5000));
[acf41,lag41]=autocorr(vcon(41,Block-10000:Block-5000));
[acf42,lag42]=autocorr(vcon(42,Block-10000:Block-5000));
[acf43,lag43]=autocorr(vcon(43,Block-10000:Block-5000));
...
plot(RR,RG20,'Color','k','LineStyle',':','LineWidth',0.5)
...
title('Heterogeneous subjective probabilities (q) from reference groups','FontSize',20)
legend('Mean violations E(v|R) common q (Baseline)','Mean violations E(v|R) heterogeneous q, ref. group = 30','Mean violations E(v|R) heterogeneous q, ref. group = 20','FontSize',15)
legend boxoff
hold off
