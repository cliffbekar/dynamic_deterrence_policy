% EPDDsearch2BinAR.m search routine for optimal 3 Bin policy using AR proxy 
...
for m=1:MC
        ...
        for i=BEd:BEu
            ...
            while ((cc<1) && (b<MM))
                ...
                for t=1:Block
                    ...
                    qb(t)=az2(t)/vz2(t);            
                    if t>1
                        if qb(t)>round(BinE(i)/(2*N),2)
                            R(t)=Rgb;
                        elseif (round(BinE(i)/(2*N),2)>=qb(t))&&(qb(t)>round(BE2/(2*N),2))
                            R(t)=Rtb;
                        elseif round(BE2/(2*N),2)>=qb(t)
                            R(t)=Rbb;
                        end
                        if R(t)~=R(t-1)
                            swi(t)=abs(Rbb-Rgb); 
        ...
        for i=BE2d:BE2u
            ...
            while ((cc<1) && (b<MM))
                ...
                for t=1:Block
                    ...
                    qb(t)=az2(t)/vz2(t);            
                    if t>1
                        if qb(t)>round(BE/(2*N),2)
                            R(t)=Rgb;
                        elseif (round(BE/(2*N),2)>=qb(t))&&(qb(t)>round(BinE2(i)/(2*N),2))
                            R(t)=Rtb;
                        elseif round(BinE2(i)/(2*N),2)>=qb(t)
                            R(t)=Rbb;
                        end
                        if R(t)~=R(t-1)
                            swi(t)=abs(Rbb-Rgb); 
    ...
    count(m)=count3;
    BnE1(m)=round(BE/(2*N),2);
    BnE2(m)=round(BE2/(2*N),2);
    RRRgb(m)=Rgb;
    RRRtb(m)=Rtb;
    RRRbb(m)=Rbb;
    CM(m)=RbbM;
