% Produces figure 7 
% EPDDattFig7.m
% Uses data from EPDDmainconvFig1.m
% Revised Jan., 2026, K. I. Carlaw 
...
[ff,vwx]=ksdensity(vw,'NumPoints',101);
[ff2,vwx2]=ksdensity(v30,'NumPoints',101);
...
for i=1:count(1)
    for j=1:2
        xvec(i,j)=0.045+(i-1)*0.009 + (j-1)*0.009;
        yvec(i,j)=0.564;
    end
end
for i=count(1)+1:NN %count(2)
    for j=1:2
        xvec(i,j)=0.07+NN*0.009 -((i-1-count(1))*0.009 + (j-1)*0.009);
        yvec(i,j)=0.564;
    end
end
...
for i=1:count30(1)
    for j=1:2
        xvec30(i,j)=0.045+(i-1)*0.009 + (j-1)*0.009;
        yvec30(i,j)=0.087;
    end
end
for i=count30(1)+1:count30(2)
    for j=1:2
        xvec30(i,j)=0.075+count30(2)*0.009 -((i-1-count30(1))*0.009 + (j-1)*0.009);
        yvec30(i,j)=0.087;
    end
end
for i=count30(2)+1:count30(3)
    for j=1:2
        xvec30(i,j)=0.069+count30(2)*0.009 + ((i-1-count30(2))*0.009 + (j-1)*0.009);
        yvec30(i,j)=0.087;
    end
end
for i=count30(3)+1:NN
    for j=1:2
        xvec30(i,j)=0.09+NN*0.009 -((i-1-count30(3))*0.009 + (j-1)*0.009);
        yvec30(i,j)=0.087;
    end
end


[acf,lag]=autocorr(vw,NumLags=10);
[acfb,lagb]=autocorr(v30,NumLags=10);
...
ylabel('E(v^{t+1})','FontSize',20)
lgd=findobj('type', 'legend');
delete(lgd);
%legend('E(v^{t+1}|v^{t})','FontSize',14,'Location','north','Visible','off') 
title('Panel 1: Dynamics of the q-attractors, (BE^{*}, R^{*}_{1}, R^{*}_{2}) = (35, 30, 57)','FontSize',25,'FontWeight','normal')
%legend boxoff
txt=['v^{t}\approx',num2str((count(1)-1),'%.2f')];
text((count(1)-1),-3,txt,'FontSize',15);
txt1=['Basin of attraction to v^{t}\approx',num2str((count(1)-1),'%.2f')];
text((count(1)+4),6,txt1,'FontSize',17);
...
for i=1:round(N/3)
    annotation('textarrow',xvec30(i*3,:),yvec30(i*3,:))
end 
xlabel('v^{t}','FontSize',20)
ylabel('E(v^{t+1})','FontSize',20)
%legend('E(v^{t+1}|v^{t})','FontSize',14,'Location','north') 
title('Panel 2: Dynamics of the q-attractors, R = 30','FontSize',25,'FontWeight','normal')
lgd=findobj('type', 'legend');
delete(lgd);
%legend boxoff
txt=['v^{t}\approx',num2str((count30(1)-1),'%.2f')];
text((count30(1)-1),-3,txt,'FontSize',15);
txt1=['v^{t}\approx',num2str((count30(2)-1),'%.2f')];
text((count30(2)-1),-3,txt1,'FontSize',15);
txt2=['v^{t}\approx',num2str((count30(3)-1),'%.2f')];
text((count30(3)-1),-3,txt2,'FontSize',15);
txt3=['Basin of attraction to v^{t}\approx',num2str((count30(1)-1),'%.2f')];
text((count30(1)-5),6,txt3,'FontSize',17);
txt4=['Basin of attraction to v^{t}\approx',num2str((count30(3)-1),'%.2f')];
text((count30(2)+40),6,txt4,'FontSize',17);
ylim([-6 N])
%legend('AutoUpdate','off')
xline(N)
yline(0)
...
handaxes1=axes('position',[0.75 0.62 0.20 0.20]);
...
handaxes2=axes('position',[0.75 0.16 0.20 0.20]);
...
handaxes3=axes('position',[0.1 0.70 0.20 0.20]);%[0.77 0.63 0.18 0.20]);
%plot(vwx/N,ff,'Color','k','LineWidth',2)
histogram(vw(100:Block),'Normalization','probability','FaceColor',[0.17 0.17 0.17],'BinWidth',1)
title('Frequency of violations')
xlabel('Violations')

handaxes4=axes('position',[0.1 0.225 0.20 0.20]);%[0.77 0.17 0.18 0.20]);
%plot(vwx2/N,ff2,'Color','k','LineWidth',2)
