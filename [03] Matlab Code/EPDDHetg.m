%EPDDHetg.m
%Person specific g distributions  for Section 3.1.1 of Online Appendix  
...
sigi=0.008;  %variance for individual g's
...
mui=zeros(N,1);PHI=zeros(N,1);
PHI(1)=0.005;
for i=2:N
    PHI(i)=PHI(i-1)+0.01;
    mui(i)=norminv(PHI(i),mu,sig);    
end 
...
for r=1:NN
    ...
    while ((cc<1) && (b<MM))
        g=zeros(Block,N);
        for i=1:N
            g(:,i)=normrnd(mui(i),sigi,Block,1);
        end
...
plot(RR,mvbl,'Color','k','LineStyle','-','LineWidth',1)
plot(RR,mv,'Color','k','LineStyle','--','LineWidth',1)
plot(RR,mvunifg,'Color','k','LineStyle',':','LineWidth',1)
...
title('Variations in the distribution for g','FontSize',20)
legend('E(v|R) normal g dsitribution (Baseline)','E(v|R) heterogeneous g dsitributions','E(v|R) uniform g dsitribution','Fontsize',15)
legend boxoff
hold off
