...
for i=count(1)+1:NN %count(2)
    for j=1:2
        xvec(i,j)=0.036+NN*0.0095 -((i-1-count(1))*0.0093 + (j-1)*0.0093);
        yvec(i,j)=0.14;
    end
end
...
%legend('E(VR^{t+1}|VR^{t})','Location','east','FontSize',14) 
%legend boxoff
txt=['v^{t}=',num2str((count(1)-1),'%.2f')];
text((count(1)-1),-3,txt,'FontSize',15);
txt3=['Basin of attraction to v^{t}\approx',num2str((count(1)-1),'%.2f')];
text((count(1)+3),4,txt3,'FontSize',17);
...
handaxes2=axes('position',[0.7 0.29 0.25 0.35]);
...
handaxes3=axes('position',[0.1 0.55 0.25 0.35]);%[0.7 0.29 0.20 0.25]);
